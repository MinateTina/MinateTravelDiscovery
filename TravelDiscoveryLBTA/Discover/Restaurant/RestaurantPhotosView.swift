    ...
    var body: some View {
        GeometryReader { proxy in
            ScrollView {
                ...
                if mode == "grid" {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: proxy.size.width / 3 - 4, maximum: 300), spacing: 4)], spacing: 2) {
                        ForEach(photoUrlStrings, id:\.self) { photoUrlString in
                            Button {
                                ...
                                WebImage(url: URL(string: photoUrlString.description))
                                    .resizable()
                                    .scaledToFill()
